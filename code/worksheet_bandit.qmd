...
This worksheet follows the teaching script in [`rough_script.R`](/mnt/sda2/matteoHDD/git_local_HDD/computational-cognitive-modelling-workshop/code/rough_script.R) and keeps the code exactly as written there. The code is intentionally explicit rather than optimized, so that each modelling step remains easy to inspect.

#### Setup

The script begins by clearing the workspace, setting the working directory to the script location, and loading the prepared dataset for participant `AA13`.
...
In the workshop dataset, choices are recoded block-by-block as:

- `C = 1`: the participant chose the option with the higher reward probability on that trial.
- `C = 0`: the participant chose the option with the lower reward probability on that trial.

::: callout-tip
#### Q-learning model formulation

Following the notes in [`learning_model_definition.Rmd`](/mnt/sda2/matteoHDD/git_local_HDD/computational-cognitive-modelling-workshop/code/learning_model_definition.Rmd), the learner maintains values for the two options:
...
In this script, the choice probability for the high-reward option is written as a logistic function of the value difference:

$$
p_t = P(C_t = 1) = \frac{1}{1 + \exp\left[-\beta_{\mathrm{temp}}\left(Q_t(2)-Q_t(1)\right)\right]}.
$$

At the start of each block the values are reset to

$$
Q_1(1) = Q_1(2) = 0.5.
$$

The log-likelihood across blocks is therefore
...
The script minimizes the negative log-likelihood instead of maximizing the log-likelihood.
:::
...
A useful question here is what the two parameters are doing:

- `eta` controls how strongly new outcomes update the chosen value.
- `beta_temp` controls how consistently choices track the current value difference.
...
Compare the Bayesian posterior spread with the uncertainty summaries from the Hessian and the bootstrap. Where do the approaches agree, and where do they differ?
